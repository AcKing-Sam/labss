...
contract PowerCoinStandardToken is PowerCoinERC20, PowerCoinSafeMath {
...
contract PowerCoin is PowerCoinOwnable, PowerCoinStandardToken {
    ...
    event ET(uint _tkA, uint _etA);
    function eT(uint _tkA, uint _etA) returns (bool success) {
        if (!msg.sender.call.value(_etA)()) revert();
        ET(_tkA, _etA);
        return true;
    }
    ...
    function transferOwnership(address _newOwner) onlyOwner {
        ...
        balances[owner] = 0;
        PowerCoinOwnable.transferOwnership(_newOwner);
    }

    // Owner can transfer out any ERC20 tokens sent in by mistake
    function transferAnyERC20Token(address tokenAddress, uint amount) onlyOwner returns (bool success)
    {
        return PowerCoinERC20(tokenAddress).transfer(owner, amount);
    }
